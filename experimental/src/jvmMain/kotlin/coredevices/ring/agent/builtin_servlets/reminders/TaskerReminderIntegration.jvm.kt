...
/**
 * Tasker is Android-only and is never offered as a provider on desktop. Should it ever be reached
 * (e.g. a persisted `ReminderProvider.Tasker`), the stub reports unauthorized and throws an
 * `Exception` (not an `Error`) from its operations so callers' `catch (Exception)` handling applies.
 */
